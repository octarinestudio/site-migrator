...
class SMIG_Source_API {
	...
	public function register_routes() {
		...
		register_rest_route( self::NAMESPACE, '/table-schema', array(
			'methods'             => 'GET',
		...
		register_rest_route( self::NAMESPACE, '/table-rows', array(
			'methods'             => 'GET',
			...
			'args'                => array(
				...
				'per_page' => array( 'default' => SMIG_ROWS_PER_PAGE, 'sanitize_callback' => 'absint' ),
			),
		) );

		register_rest_route( self::NAMESPACE, '/files-list', array(
			'methods'             => 'GET',
		...
		register_rest_route( self::NAMESPACE, '/file-content', array(
			'methods'             => 'GET',
	...
	public function endpoint_manifest() {
		...
		foreach ( $tables as $t ) {
			...
			$table_info[] = array( 'name' => $t, 'rows' => $rows );
		}
		...
		foreach ( array( 'uploads', 'theme', 'plugins' ) as $type ) {
			...
			$file_types[ $type ] = array_merge( array(
				'count' => count( $list ),
				'size'  => $total_size,
			), $extra );
